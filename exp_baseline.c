...
static int RunPhase5_OCV(BaselineExperimentContext *ctx);
...
static const char* GetPhaseDescription(BaselineExperimentPhase phase) {
    switch (phase) {
        case BASELINE_PHASE_0: return "Phase 0: Pre-Experiment OCV";
        ...
        case BASELINE_PHASE_5: return "Phase 5: Post-Experiment OCV";
...
static const char* GetStateDescription(BaselineExperimentState state) {
    switch (state) {
        ...
        case BASELINE_STATE_PHASE5_OCV: return "Phase 5 OCV";
...
int CVICALLBACK StartBaselineExperimentCallback(int panel, int control, int event,
    ...
    g_experimentContext.params.ocvRestTime *= 60.0;  // UI is in minutes, internal is seconds
#else
    g_experimentContext.params.runOCVPhase = 0;
    g_experimentContext.params.ocvRestTime = OCV_REST_DEFAULT_TIME * 60.0;  // Convert default minutes to seconds
#endif

#ifdef BASELINE_CHK_OCV_POST
    GetCtrlVal(panel, BASELINE_CHK_OCV_POST, &g_experimentContext.params.runOCVPost);
#else
    g_experimentContext.params.runOCVPost = 0;
#endif
...
static int BaselineExperimentThread(void *functionData) {
    ...
    snprintf(message, sizeof(message),
        ...
        ctx->params.runOCVPhase ? "0. Pre-experiment OCV measurement\n" : "",
        ENABLE_DTB ? " and establish temperature" : "",
        ctx->params.runOCVPost ? "5. Post-experiment OCV measurement" : "");
    ...
    // PHASE 5: Post-Experiment OCV Measurement (optional)
    if (ctx->params.runOCVPost) {
        LogMessage("=== PHASE 5: Post-Experiment OCV Measurement ===");
        ctx->currentPhase = BASELINE_PHASE_5;
        ctx->state = BASELINE_STATE_PHASE5_OCV;
        SetCtrlVal(ctx->tabPanelHandle, ctx->statusControl, "Phase 5: Measuring post-experiment OCV...");

        result = RunPhase5_OCV(ctx);
        if (result != SUCCESS || CheckCancellation(ctx)) {
            if (!CheckCancellation(ctx)) {
                ctx->state = BASELINE_STATE_ERROR;
            }
...
static int SaveExperimentSettings(BaselineExperimentContext *ctx) {
    ...
    WriteINIValue(file, "Phase0_OCV_Enabled", "%d", ctx->params.runOCVPhase);
    WriteINIValue(file, "Phase5_OCV_Enabled", "%d", ctx->params.runOCVPost);
    WriteINIDouble(file, "OCV_Rest_Time_s", ctx->params.ocvRestTime, 0);
    fprintf(file, "\n");
...
static int RunPhase5_OCV(BaselineExperimentContext *ctx) {
    LogMessage("Phase 5: Running post-experiment OCV measurement");
    ...
    memset(&ctx->phase5Result, 0, sizeof(ctx->phase5Result));

    int result = OCV_RunExperimentInDir(&ocvParams,
                                        ctx->experimentDirectory,
                                        BASELINE_PHASE5_DIR,
                                        "Phase 5",
                                        ctx->statusControl,
    ...
    if (result == SUCCESS) {
        LogMessage("Phase 5 complete: OCV = %.4f V", ctx->phase5Result.finalOCV_V);

        // Switch back to PSB after completion
        int switchResult = SwitchToPSB(ctx);
        if (switchResult != SUCCESS) {
            LogError("Phase 5: Failed to switch back to PSB after OCV measurement");
        }
    } else {
        LogError("Phase 5: OCV measurement failed with error %d", result);
    }
...
static int RunPhase3_EISCharge(BaselineExperimentContext *ctx) {
    ...
    // - REG 501 (source current) = chargeCurrent (NON-ZERO!)
    // - REG 499 (sink current) = dischargeCurrent
    // - REG 500 (voltage) = targetVoltage
    // - REG 502/498 (power) = 20W from initialization
    //
    // The "zero REG 501" approach was an INCORRECT hypothesis that caused zero current.
    LogMessage("=== Configuring PSB for Phase 3 charging (original working pattern) ===");
    LogMessage("  Target: V=%.3fV, I=%.3fA", ctx->params.chargeVoltage, ctx->params.chargeCurrent);
    LogMessage("  Power limits: 20W (set by initialization)");

    // Set source current (REG 501) - CRITICAL: must be non-zero!
    LogMessage("Setting source current (REG 501) to %.3f A", ctx->params.chargeCurrent);
    ...
    // Set sink current (REG 499) - for discharge direction (use same value)
    LogMessage("Setting sink current (REG 499) to %.3f A", ctx->params.dischargeCurrent);
    result = PSB_SetSinkCurrentQueued(ctx->params.dischargeCurrent, DEVICE_PRIORITY_NORMAL);
    if (result != PSB_SUCCESS) {
    ...
    LogMessage("Setting voltage (REG 500) to %.3f V", ctx->params.chargeVoltage);
...
static int ProcessGEISData(BIO_TechniqueData *geisData, BaselineEISMeasurement *measurement) {
    ...
    // Direct DLL mode: process 1 with 11+ variables [freq, ..., Re(Zwe), Im(Zwe), ...]
    if (processIndex == 1 && convData->numVariables >= 11) {
        LogDebug("Processing Direct DLL mode GEIS data");
        ...
        LogDebug("Successfully extracted %d impedance points from Direct DLL GEIS data",
                 measurement->numPoints);
    }
    // EC-Lab mode: process 0 with 4 variables [time, freq, Re(Z), -Im(Z)]
    else if (processIndex == 0 && convData->numVariables == 4) {
        LogDebug("Processing EC-Lab mode GEIS data");

        // Extract impedance data from EC-Lab format
        for (int i = 0; i < convData->numPoints; i++) {
            measurement->frequencies[i] = convData->data[1][i];  // Frequency (index 1)
            measurement->zReal[i] = convData->data[2][i];       // Re(Z) (index 2)
            measurement->zImag[i] = -convData->data[3][i];      // Im(Z) = -(-Im(Z)) (index 3)
        }

        measurement->numPoints = convData->numPoints;

        LogDebug("Successfully extracted %d impedance points from EC-Lab GEIS data",
                 measurement->numPoints);
    }
    else {
...
static void CleanupExperiment(BaselineExperimentContext *ctx) {
    ...
    // Free Phase 0 and Phase 5 OCV result data
    OCV_FreeResult(&ctx->phase0Result);
    OCV_FreeResult(&ctx->phase5Result);
